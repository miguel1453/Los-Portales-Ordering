...
//            }
//        
//        Firestore.firestore().collection("cakOrders \(cakePickupDate)").document()
//            .setData(data) { error in
//                if let error = error {
//                    print("DEBUG: FAILED to upload Cake Order with error \(error.localizedDescription)")
//                    completion(false)
...
//    }
//    
//    func fetchDayCakes(with day: String, completion: @escaping([Cake]) -> Void) {
//        Firestore.firestore().collection("cakeOrders \(day)")
//            .order(by: "cakePickupTime")
//            .getDocuments { snapshot, _ in
//                guard let documents = snapshot?.documents else {
//                    print("DEBUG: error fetching \(day) cakes")
//                    return
//                }
//                let cakes = documents.compactMap({ try? $0.data(as: Cake.self)})
//                
